...
<p align="center"><img src="docs/assets/report-preview.png" alt="A driln markdown report rendered on GitHub, with a Mermaid pie chart and correlation graph" width="100%"></p>

*A real markdown report generated by `driln report` — screenshotted, not mocked up. The scan
above only turned up open-port info on a low-risk test target, so this one uses example
findings run through the same dedup/correlation/risk-scoring pipeline, to actually show the
pie chart and correlation graph doing something.*
