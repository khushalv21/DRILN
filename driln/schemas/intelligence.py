...
class TechFingerprint(BaseModel):
    ...
    confidence: float = Field(1.0, ge=0.0, le=1.0, description="Detection confidence (0.0-1.0)")
    sources: list[str] = Field(default_factory=list, description="Tools that detected this")
...
class TechProfile(BaseModel):
    ...
    servers: list[str] = Field(default_factory=list, description="Web/app servers detected")
    frameworks: list[str] = Field(default_factory=list, description="CMS/frameworks detected")
    languages: list[str] = Field(default_factory=list, description="Programming languages detected")
    os_hints: list[str] = Field(default_factory=list, description="OS detection hints")
...
class RiskScore(BaseModel):
    ...
    base_severity: float = Field(..., ge=0.0, le=1.0, description="Normalized base severity")
    exploitability: float = Field(..., ge=0.0, le=1.0, description="How easy to exploit")
    exposure: float = Field(..., ge=0.0, le=1.0, description="Exposure factor (internet-facing, etc.)")
    context_boost: float = Field(..., ge=0.0, le=1.0, description="Boost from correlated context")
    label: str = Field(..., description="Human-readable label: critical/high/medium/low/informational")
...
class CorrelationGroup(BaseModel):
    ...
    combined_risk: float = Field(0.0, ge=0.0, le=100.0, description="Combined risk score of the group")
...
class ScanIntelligence(BaseModel):
    ...
    finding_risk_scores: dict[str, float] = Field(default_factory=dict, description="finding_id → risk_score")
