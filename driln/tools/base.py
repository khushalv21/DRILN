...
class BaseTool(ABC):
    ...
    async def run(
        ...
        if extra_args:
            for arg in extra_args:
                if arg.startswith("-") and arg not in self.allowed_extra_args:
                    allowed = ", ".join(self.allowed_extra_args) if self.allowed_extra_args else "None"
                    raise ToolExecutionError(
                        f"Disallowed extra argument '{arg}' for tool '{self.name}'. Allowed flags: {allowed}"
                    )

        # 1. Check installation
        installed, path = await self.check_installed()
        if not installed:
            raise ToolNotFoundError(f"Tool '{self.name}' not found. Ensure '{self.binary}' is in PATH.")
        ...
        try:
            ...
            raise ToolTimeoutError(f"Tool '{self.name}' timed out after {timeout}s") from exc
        ...
        try:
            ...
            raise ToolExecutionError(f"Failed to parse output from '{self.name}': {exc}") from exc
