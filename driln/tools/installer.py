...
def _get_ssl_context() -> ssl.SSLContext:
    ...
    raise RuntimeError("Cannot create a verified SSL context. Install certifi to fix: pip install certifi")
    ...
    if url.endswith(".zip"):
        ...
        with tarfile.open(fileobj=io.BytesIO(data), mode="r:gz") as tf:
            for tar_member in tf.getmembers():
                if os.path.basename(tar_member.name) == binary_name:
                    f = tf.extractfile(tar_member)
                    if f is None:
                        raise RuntimeError(f"Cannot read '{binary_name}' from archive")
