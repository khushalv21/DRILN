...
[bold red]    ╚═════╝ ╚═╝  ╚═╝╚═╝╚══════╝╚═╝  ╚═══╝[/bold red]
"""
_TAGLINE = "[dim red]    Find it before they do.[/dim red]"
...
def _show_banner(
    ...
    if ctx.invoked_subcommand is None:
        ...
        console.print("    [white]scan[/white]      Run a penetration test scan")
        console.print("    [white]tools[/white]     List and check pentesting tools")
        console.print("    [white]report[/white]    Generate a report for a completed scan")
        console.print("    [white]intel[/white]     Scan intelligence and recommendations")
        console.print("    [white]setup[/white]     Download and install pentesting tools")
        console.print("    [white]serve[/white]     Start the API server")
        console.print()
        console.print("  [dim]Run[/dim] [bold]driln <command> --help[/bold] [dim]for details.[/dim]")
        console.print("  [dim]Run[/dim] [bold]driln --version[/bold] [dim]to show version.[/dim]")
        console.print()
...
def _validate_target(target: str, allow_local: bool = False) -> str:
    ...
    """
    from driln.core.validation import TargetValidationError, validate_target_format

    try:
        target = validate_target_format(target)
    except TargetValidationError as e:
        console.print(f"  [bold white]Error:[/bold white] {e}")
    ...
    if not allow_local:
        ...
        if resolve_and_check_local(target):
            console.print("[white]  Error: Target resolves to a private IP.[/white]")
...
def _header(title: str, subtitle: str = "") -> Panel:
    ...
    if subtitle:
        content += f"\n[dim]{subtitle}[/dim]"
    return Panel(content, box=box.ROUNDED, style="white", expand=False)


# ── Sub-commands ─────────────────────────────────────────────────

tools_app = typer.Typer(help="Manage pentesting tools", invoke_without_command=True)


@tools_app.callback()
def tools_callback(ctx: typer.Context) -> None:
    """When `driln tools` is run with no subcommand, default to `list`."""
    if ctx.invoked_subcommand is None:
        tools_list()
...
def serve(
    ...
    console.print(f"  [dim]Swagger UI →[/dim] [white]http://{host}:{port}/docs[/white]")
    console.print(f"  [dim]Health     →[/dim] [white]http://{host}:{port}/health[/white]\n")
    uvicorn.run(
        "driln.main:app",
        host=host,
        port=port,
...
def scan(
    ...
    allow_local: bool = typer.Option(
        False, "--allow-local", help="Allow scanning of internal/private IP addresses"
    ),
    ...
    if config:
        ...
        if not config_path.exists():
            console.print(f"[white]Error: Config file {config} not found[/white]")
            raise typer.Exit(1)
        try:
            config_dict = json.loads(config_path.read_text())
        except Exception as e:
            console.print(f"[white]Error parsing config file: {e}[/white]")
            raise typer.Exit(1)
...
async def _run_scan(
    ...
    console.print()
    sev_suffix = f" · {sev_str}" if sev_str else ""
    console.print(f"  [bold green]✓ Scan complete[/bold green] · {total_findings} findings{sev_suffix}")
    console.print(f"  [dim]Report:[/dim] [white]{result['filepath']}[/white]")
...
async def _tools_list() -> None:
    ...
    tool_descriptions = {
        "subfinder": "Discovers subdomains for a target domain.",
        "httpx": "Probes discovered hosts to identify live web servers.",
        "nmap": "Scans live hosts for open ports and running services.",
        "nuclei": "Runs vulnerability templates against live targets.",
    }

    table = Table(box=box.ROUNDED, show_lines=False, padding=(0, 1))
    table.add_column("Tool", style="bold white", min_width=12)
    ...
    for info in results.values():
        if info["installed"]:
            ...
            status = "[bold white]○ Missing[/bold white]"

        purpose = tool_descriptions.get(info["name"].lower(), "—")
        table.add_row(
            info["name"],
            purpose,
            info["binary"],
...
async def _tools_check() -> None:
    ...
    for info in results.values():
        if info["installed"]:
            console.print(f"  [bold green]✓[/bold green] [white]{info['name']}[/white] — {info['path']}")
        else:
            console.print(f"  [bold white]✗[/bold white] [white]{info['name']}[/white] — not found in PATH")
            all_ok = False
...
def report(
    ...
    """Generate a markdown report for a completed scan."""
...
async def _generate_report(scan_id: str, no_ai: bool) -> None:
    ...
    console.print(
        Panel(
            f"[dim]File: [/dim] [white]{result['filepath']}[/white]",
    ...
    async def _run() -> None:
        ...
        async with factory() as session:
            ...
            if intel.tech_profile.technologies:
                ...
                tech_table.add_column("Technology", style="bold white")
            ...
            if intel.correlation_groups:
                ...
                for g in intel.correlation_groups:
                    rel_color = {
                        ...
                        "attack_chain": "red",
                        "tech_overlap": "yellow",
                    }.get(g.relationship, "white")
                    rel_tag = f"[{rel_color}]{g.relationship}[/{rel_color}]"
                    console.print(
                        f"    [{rel_color}]●[/{rel_color}] {rel_tag}  {g.summary}"
                        f"  [dim]({len(g.finding_ids)} findings)[/dim]"
                    )
            ...
            if intel.recommendations:
                ...
                rec_table.add_column("Tool", width=12, style="white")
            ...
            # Footer stats
            console.print(
                f"\n  [dim]Deduplicated: {intel.deduplicated_count}  |  "
                f"Enriched: {intel.enriched_count}  |  Scan: {scan_id[:8]}…[/dim]\n"
            )
        ...
        async with factory() as session:
            ...
            for idx, r in enumerate(recs, 1):
                ...
                status = (
                    ...
                    else "[bold white]✗ Dismissed[/bold white]"
...
def setup() -> None:
    ...
    for name, ok in results.items():
        if ok:
            ...
            status = "[bold white]✗ Failed[/bold white]"
