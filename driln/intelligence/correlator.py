...
class FindingCorrelator:
        ...
        for key, group_findings in buckets.items():
            ...
            for f in group_findings:
                tool = f.get("tool_name") or (
                    f.get("source_tools", ["unknown"])[0] if f.get("source_tools") else "unknown"
                )
                tools.add(tool)
    ...
    def _by_attack_chain(self, findings: list[dict[str, Any]]) -> list[CorrelationGroup]:
        ...
        service_findings: dict[str, dict[str, list[dict[str, Any]]]] = defaultdict(lambda: defaultdict(list))
        ...
        for key, by_sev in service_findings.items():
            ...
            high_tier = by_sev.get("medium", []) + by_sev.get("high", []) + by_sev.get("critical", [])

            if low_tier and high_tier:
                all_ids = [f.get("id", "") for f in low_tier + high_tier if f.get("id")]
                if len(all_ids) < 2:
                    continue
                ...
                groups.append(
                    CorrelationGroup(
                        ...
                        summary=(f"Service info + vulnerability on {key}: {', '.join(high_titles)}"),
                        combined_risk=0.0,
                    )
                )
        return groups
    ...
    def _by_technology(
        ...
        tech_buckets: dict[str, list[dict[str, Any]]] = defaultdict(list)
        for f in findings:
