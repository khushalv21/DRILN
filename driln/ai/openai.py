...
class OpenAIProvider(BaseAIProvider):
    ...
    async def complete(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise AIConnectionError(f"Cannot connect to AI provider at {self._base_url}") from exc
        except httpx.TimeoutException as exc:
            raise AIConnectionError("AI provider request timed out") from exc

        if response.status_code != 200:
            raise AIResponseError(f"AI provider returned {response.status_code}: {response.text[:500]}")
    ...
    async def analyze_scan(self, scan_data: dict[str, Any]) -> str:
        ...
        if not self._api_key:
            return (
                "_AI analysis skipped — no API key provided. Add your own key to "
                ".env (DRILN_AI_API_KEY) to enable AI summaries._"
            )
    ...
    async def structured_analyze(self, scan_data: dict[str, Any]) -> AIStructuredAnalysis:
