...
class AIFindingAnalysis(BaseModel):
    ...
    context: str = Field(..., description="Why this finding matters for this specific target")
...
class AIAttackPath(BaseModel):
    ...
    steps: list[str] = Field(..., description="Ordered list of attack steps")
    finding_titles: list[str] = Field(..., description="Finding titles involved in this path")
    likelihood: str = Field(..., pattern=r"^(high|medium|low)$")
    impact: str = Field(
        ...,
...
class AIRecommendation(BaseModel):
    ...
    tool_name: str | None = Field(None, description="Suggested tool to run, or null if manual")
    tool_config: dict[str, Any] | None = Field(None, description="Suggested tool configuration")
...
class AIStructuredAnalysis(BaseModel):
    ...
    executive_summary: str = Field(..., description="2-3 sentence executive summary of security posture")
