...
def register_provider(name: str, cls: type[BaseAIProvider]) -> None:
    ...
    if not (isinstance(cls, type) and issubclass(cls, BaseAIProvider)):
        raise TypeError(f"Provider class must inherit from BaseAIProvider, got {cls}")
...
def _load_custom_provider() -> None:
    ...
    try:
        ...
        raise ConfigError(f"Error loading custom AI provider module '{module_name}': {e}") from e
...
def _create_provider(name: str) -> BaseAIProvider:
    ...
    raise ConfigError(f"Unknown AI provider: '{name}'. Available: {', '.join(available)}")
