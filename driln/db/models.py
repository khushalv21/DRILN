...
class Finding(Base):
    ...
    correlation_group: Mapped[str | None] = mapped_column(String(36), nullable=True, index=True)
    deduplicated: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    source_tools: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)
...
class Recommendation(Base):
    ...
    source: Mapped[str] = mapped_column(String(30), nullable=False)  # "rule_engine" | "ai_analysis"
    accepted: Mapped[bool | None] = mapped_column(Boolean, nullable=True)  # None=pending
    triggered_by: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)  # finding IDs
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utcnow, nullable=False)
