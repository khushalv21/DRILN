...
def test_recommendation_uuid_validation():
    ...
    assert response.status_code == 400
    assert (
        "Invalid id format" in response.json()["detail"]
        or "Invalid rec_id format" in response.json()["detail"]
    )
...
def test_scan_create_extra_args_injection():
    ...
    payload = {
        ...
        "scan_type": "full",
        "tools": ["nmap"],
        "config": {"nmap": {"extra_args": ["--script-args", "os.execute('rm -rf /')"]}},
    }
...
def test_scan_create_target_validation():
    ...
    payload = {"target": "; rm -rf /", "scan_type": "full"}
    response = client.post("/api/v1/scans", json=payload)
    assert response.status_code == 422  # Pydantic validation error
...
def test_scan_create_ssrf_protection():
    ...
    payload = {"target": "127.0.0.1", "scan_type": "full"}
    response = client.post("/api/v1/scans", json=payload)
    assert response.status_code == 400
...
def test_scan_create_ssrf_allow_local():
    ...
    payload = {"target": "127.0.0.1", "scan_type": "full", "allow_local": True}
