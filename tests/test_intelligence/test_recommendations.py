...
from driln.intelligence.context import HostContext, ScanContext, ServiceContext
from driln.intelligence.recommendations import (
    RecommendationEngine,
    _has_finding_keyword,
    _has_open_port,
    _has_service,
    _has_tech,
)
...
def test_has_tech():
    ...
    assert _has_tech(tp, "wordpress")
    assert not _has_tech(tp, "joomla")
...
def test_has_service(empty_context):
    ...
    assert _has_service(ctx, "smb", "microsoft-ds")
    assert not _has_service(ctx, "ftp")


def test_has_finding_keyword(empty_context):
    ctx = empty_context
    ctx.findings = [{"title": "Admin panel login detected"}, {"title": "Random finding"}]
    assert _has_finding_keyword(ctx, "admin", "login")
    assert not _has_finding_keyword(ctx, "sql")
...
def test_has_open_port(empty_context):
    ...
    assert _has_open_port(ctx, 3306)
    assert not _has_open_port(ctx, 5432)
